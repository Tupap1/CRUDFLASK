    ...
    let url = "http://127.0.0.1:5000/";
                ...
                const visualizar = (data) => {
                    ...
                    for (var i = 0; i < data.baul.length; i++) {
                        ...
                        b+=`<tr><td>${data.baul[i].id_baul}</td><td>${data.baul[i].Plataforma}</td><td>${data.baul[i].usuario}</t
   d><td>${data.baul[i].clave}</td>
                        <td>
<button type='button' class="btn btninfo" onclick="location.href ='edit.html?variable1=${data.baul[i].id_baul}'">
<h5>Actualizar<h5/> </button>
                        
<button type='button' class="btn btnwarning" onclick="eliminar(${data.baul[i].id_baul})">
 <h5>Borrar</h5>
   </button></tr>` }
